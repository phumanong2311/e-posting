...
const TopCompaniesByJobPosts = () => {
  ...
  const fetchJobs = async () => {
    ...
    while (true) {
      const allJobsResponse = await jobServices.getListJob({
        page: currentPage,
      });
    ...
    allJobPosters.forEach((job: Job) => {
      ...
      if (!companyName || companyName === "") {
        companyCounts["No Company Name"] =
          (companyCounts["No Company Name"] || 0) + 1;
      } else {
        companyCounts[companyName] = (companyCounts[companyName] || 0) + 1;
      }
    });

    const companyCountsArray = Object.entries(companyCounts).map(
      ([name, count]) => ({ name, count })
    );
    companyCountsArray.sort((a, b) => b.count - a.count);

    const top20Companies = companyCountsArray
      .slice(0, 20)
      .map((company, index) => ({
        rank: index + 1,
        ...company,
