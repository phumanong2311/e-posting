...
class RequestService extends API {
  ...
  async getTopRequestPosters(
    month: number,
    year: number,
    startDate?: Date,
    endDate?: Date
  ): Promise<{ requestOwner: string; numPostings: number }[]> {
    let currentPage = 1;
    let allRequestPosters: any[] = [];

    while (true) {
      const allRequestsResponse = await this.getRequests({
        searchParameter: {},
        page: currentPage,
      });
    ...
    if (startDate) {
      allRequestPosters = allRequestPosters.filter(
        (request: any) => new Date(request.createdAt) >= startDate
      );
    }
    if (endDate) {
      allRequestPosters = allRequestPosters.filter(
        (request: any) => new Date(request.createdAt) <= endDate
      );
    }
    ...
    const topRequestPosters = Object.keys(requestPosters).map(
      (requestOwner) => ({
        requestOwner,
