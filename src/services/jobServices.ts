...
class JobService extends API {
  ...
  async getJobSearch({
    ...
    let url = "job?" + buildQueryParams({ ...searchParameter, page });
    if (keyword) {
      url =
        "job/search?" + buildQueryParams({ ...searchParameter, keyword, page });
    }
    return this.getAPI(url);
  }
  ...
  async getTopJobPosters(
    month: number,
    year: number,
    startDate?: Date,
    endDate?: Date
  ): Promise<{ jobOwner: string; numPostings: number }[]> {
    let currentPage = 1;
    let allJobPosters: any[] = [];

    while (true) {
      const allJobsResponse = await this.getJobs({
        searchParameter: {},
        page: currentPage,
      });
      ...
      const currentMonthJobs = jobs.filter((job: any) => {
        ...
        return (
          jobDate.getMonth() + 1 === month && jobDate.getFullYear() === year
        );
      });
    ...
    if (startDate) {
      allJobPosters = allJobPosters.filter(
        (job: any) => new Date(job.createdAt) >= startDate
      );
    }
    if (endDate) {
      allJobPosters = allJobPosters.filter(
        (job: any) => new Date(job.createdAt) <= endDate
      );
    }
